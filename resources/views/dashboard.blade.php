...
@section('content')
    <div class="alert alert-info border-0 shadow-sm rounded-3 mb-4 bg-white border-start border-4 border-info">
        <div class="d-flex align-items-center gap-3 py-2">
            <div class="text-info fs-3 bg-info bg-opacity-10 rounded-circle p-2 d-flex align-items-center justify-content-center" style="width: 48px; height: 48px;">
                <i class="bi bi-person-workspace"></i>
            </div>
            <div>
                <h4 class="mb-1 text-dark fw-bold">Bonjour, Dr. {{ auth()->user()->name }}</h4>
                <p class="mb-0 text-muted">Voici le résumé quotidien de votre cabinet médical.</p>
            </div>
        </div>
    </div>
                                    ...
                                    <tr>
                                        ...
                                        <td>
                                            <div class="d-flex align-items-center gap-2">
                                                ...
                                                @if($apt->status === 'pending' && !auth()->user()->isPatient())
                                                    <form action="{{ route('appointments.confirm', $apt) }}" method="POST" class="d-inline m-0 p-0">
                                                        @csrf
                                                        @method('PATCH')
                                                        <button type="submit" class="btn btn-sm text-success p-0" title="Confirmer">
