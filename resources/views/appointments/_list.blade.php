    ...
    <tr>
        ...
        <td class="text-end">
            ...
            @if($apt->status === 'pending' && !auth()->user()->isPatient())
                <form action="{{ route('appointments.confirm', $apt) }}" method="POST" class="d-inline">
                    @csrf
                    @method('PATCH')
                    <button type="submit" class="btn btn-sm btn-light text-success mx-1" title="Confirmer le RDV">
