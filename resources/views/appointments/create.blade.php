@extends(auth()->user()->isPatient() ? 'layouts.patient' : 'layouts.app')
...
<div class="card border-0 shadow-sm rounded-4">
    <div class="card-body p-4">
        <form action="{{ route('appointments.store') }}" method="POST">
            @csrf
            <div class="row g-4">
                <div class="col-md-6" @if(auth()->user()->isPatient()) style="display: none;" @endif>
                    <label class="form-label fw-semibold">{{ __('messages.patient') }}</label>
                    <select name="patient_id" class="form-select border-0 bg-light" @if(!auth()->user()->isPatient()) required @endif>
                        <option value="">Choisir un patient...</option>
                        @foreach($patients as $p)
                            <option value="{{ $p->id }}" @if(auth()->user()->id == $p->id) selected @endif>{{ $p->name }}</option>
                        @endforeach
                    </select>
                </div>
                ...
                <div class="col-md-6" @if(auth()->user()->isPatient()) style="display: none;" @endif>
                    ...
                    <select name="status" class="form-select border-0 bg-light" @if(!auth()->user()->isPatient()) required @endif>
